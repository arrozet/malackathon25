...
# =============================================================================
# Script de Deploy AUTOMÁTICO para Brain - Malackathon 2025 (CI/CD)
# =============================================================================
# Este script despliega la aplicación en modo PRODUCCIÓN con HTTPS
# Versión NO INTERACTIVA para uso en CI/CD (GitHub Actions)
# =============================================================================
...
echo -e "${BLUE}"
echo "╔═══════════════════════════════════════════════════════════╗"
echo "║        Production Deploy - CI/CD (Malackathon 2025)       ║"
echo "╚═══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
# =============================================================================
# 2. Verificar DNS (sin interacción)
# =============================================================================
...
if [ "$SERVER_IP" != "$DOMAIN_IP" ]; then
    echo -e "${YELLOW}⚠  El DNS no apunta a este servidor${NC}"
    echo -e "${YELLOW}  Configurar DNS: $DOMAIN → $SERVER_IP${NC}"
    echo -e "${YELLOW}  Continuando de todas formas (modo CI/CD)...${NC}"
else
...
# =============================================================================
# 3. Configurar firewall (Oracle Cloud + UFW) - SIN INTERACCIÓN
# =============================================================================
...
# Recordatorio para Oracle Cloud (sin pausa)
echo -e "${YELLOW}ℹ  Oracle Cloud Security List debe estar configurado:${NC}"
echo "   - Ingress Rules: 0.0.0.0/0, TCP, Ports 80 y 443"
echo -e "${GREEN}✓ Asumiendo configuración correcta (modo CI/CD)${NC}"
...
if curl -s -f -k https://$DOMAIN/health > /dev/null 2>&1; then
    ...
    echo -e "${YELLOW}⚠  HTTPS no responde (puede tardar unos segundos)${NC}"
    echo "   Revisar logs: docker-compose -f docker-compose.prod.yml logs nginx"
fi
