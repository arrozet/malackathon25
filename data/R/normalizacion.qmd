...
centros_tbl <- datos_brutos %>%
  ...
  transmute(ID_Centro = sql_quote(`Centro Recodificado`)) %>%
  arrange(ID_Centro)
...
ingresos_tbl <- datos_brutos %>%
  mutate(
    ...
    ID_Centro = sql_quote(`Centro Recodificado`),
    ID_GRD_APR = sql_quote(`GRD APR`),
  ...
  transmute(
    ...
    Fecha_de_Ingreso = sql_quote(format(as_date(`Fecha de Ingreso`), "%Y-%m-%d")),
    Fecha_de_Fin_Contacto = sql_quote(format(as_date(`Fecha de Fin Contacto`), "%Y-%m-%d")),
    Fecha_de_Inicio_contacto = sql_quote(format(Fecha_de_Inicio_contacto, "%Y-%m-%d %H:%M:%S")),
    Circunstancia_de_Contacto = sql_quote(`Circunstancia de Contacto`),
    Tipo_Alta = sql_quote(`Tipo Alta`),
    Servicio = sql_quote(`Servicio`),
    Regimen_Financiacion = sql_quote(`Régimen Financiación`),
    Procedencia = sql_quote(`Procedencia`),
    Continuidad_Asistencial = sql_quote(`Continuidad Asistencial`),
    ...
    Mes_de_Ingreso = sql_quote(format(as_date(`Mes de Ingreso`), "%Y-%m-%d")),
    Mes_Nombre_Ingreso = sql_quote(`Mes_Nombre_Ingreso`),
    Dia_Semana_Ingreso = sql_quote(`Dia_Semana_Ingreso`),
    Coste_APR = `Coste APR`,
...
dim_centros <- datos %>%
  ...
  transmute(
    ID_Centro = sql_quote(centro_recodificado)
  ) %>%
  arrange(ID_Centro)
